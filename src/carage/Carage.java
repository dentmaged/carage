...
import carage.engine.game.Game;
...
public class Carage extends Game {
    ...
    private void updateLightSource() {
        ...
        /*
         * gouraudShader.bind(); light.sendToShader(gouraudShader);
         */
        phongShader.bind();
    ...
    private void adjustLightSource() {
        if (lightIncrease) {
            light.setIntensity(light.getIntensity() + 0.1f * delta);
        }
        if (lightDecrease) {
            light.setIntensity(light.getIntensity() - 0.1f * delta);
        }
    ...
    /**
     * Get the time in milliseconds From: http://ninjacave.com/lwjglbasics4
     * 
